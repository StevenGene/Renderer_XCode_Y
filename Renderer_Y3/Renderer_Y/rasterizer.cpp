...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
 ...
 // ------------ hw 2 implement --------------
    ...
    /*
            ...
            {-1, 0.5, -5}
     */
        float xmin = std::min(t.v[0].x(), t.v[1].x());
        xmin = std::min(xmin, t.v[2].x());
        float xmax = std::max(t.v[0].x(), t.v[1].x());
        xmax = std::max(xmax, t.v[2].x());

        float ymin = std::min(t.v[0].y(), t.v[1].y());
        ymin = std::min(xmin, t.v[2].y());
        float ymax = std::max(t.v[0].y(), t.v[1].y());
...
//                float ret3 = cp.cross(a - c).z();
                ...
                Eigen::Vector2i pti(i,j);
                if (true /*|| (ret1 > 0 && ret2 > 0 && ret3 > 0) || (ret1 < 0 && ret2 < 0 && ret3 < 0)  */)  {
    ...
    // ------- net hw ---------
//    rasterize_triangle_fast(t, view_pos);
    
}

void rst::rasterizer::rasterize_triangle_fast(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos){
    auto v = t.toVector4();
